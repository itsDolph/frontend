...
export function useUser(uid) {
  ...
  const query = () => apiRequest(`/auth/exists`, "POST", {});
...
export function updateUser(uid, data) {
  // Send API request
  return apiRequest(`user-update?uid`, "PUT", data).then((user) => {
    const cacheKey = ["user", { uid }];
...
export function checkUser() {
  return apiRequest("/auth/exists", "GET");
}
...
export function getVideosList() {
  return apiRequest("/api/videos", "GET");
}

export function scrapeVideo(data) {
  return apiRequest("/api/scrape", "POST", data);
}
